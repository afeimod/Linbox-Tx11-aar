...
#ifndef _PATCHELF_TPP
#define _PATCHELF_TPP
...
template class Elf32File;
template class Elf64File;
...
[[nodiscard]] static ElfType getElfType(const FileContents &fileContents) {
    ...
    // FIXME: endianness
    return ElfType{is32Bit, is32Bit ? (reinterpret_cast<Elf32_Ehdr *>(contents))->e_machine
...
void ElfFile<ElfFileParamNames>::sortShdrs() {
    ...
    for (unsigned int i = 1; i < rdi(hdr()->e_shnum); ++i)
        if (rdi(shdrs.at(i).sh_link) != 0)
            linkage[getSectionName(shdrs.at(i))] = getSectionName(
                    shdrs.at(rdi(shdrs.at(i).sh_link)));
...
void ElfFile<ElfFileParamNames>::shiftFile(unsigned int extraPages, size_t startOffset,
    ...
    memmove(fileContents->data() + startOffset + shift, fileContents->data() + startOffset,
            oldSize - startOffset);
    ...
    for (int i = 0; i < rdi(hdr()->e_phnum); ++i) {
        ...
        if (p_start <= startOffset && p_start + rdi(phdrs.at(i).p_filesz) > startOffset &&
        ...
        if (p_start >= startOffset) {
            ...
            if (rdi(phdrs.at(i).p_align) != 0 &&
                (rdi(phdrs.at(i).p_vaddr) - rdi(phdrs.at(i).p_offset)) % rdi(phdrs.at(i).p_align) !=
                0) {
...
ElfFile<ElfFileParamNames>::tryFindSectionHeader(const SectionName &sectionName) const {
...
void ElfFile<ElfFileParamNames>::rewriteSectionsLibrary() {
    ...
    off_t headerTableSpace = roundUp(rdi(hdr()->e_shnum) * rdi(hdr()->e_shentsize),
                                     sectionAlignment);
...
void ElfFile<ElfFileParamNames>::normalizeNoteSegments() {
    ...
    for (auto &phdr: phdrs) {
        ...
        bool empty = std::none_of(shdrs.begin(), shdrs.end(),
                                  [&](auto &shdr) {
                                      return rdi(shdr.sh_offset) >= start_off &&
                                             rdi(shdr.sh_offset) < end_off;
                                  });
        if (empty)
            continue;
    ...
    if (shdrDynamic) {
        ...
        for (auto dyn = dyn_table; (d_tag = rdi(dyn->d_tag)) != DT_NULL; dyn++)
            if (d_tag == DT_STRTAB)
                ...
                /* 64-bit Linux, x86-64 */
                if (!shdr)
                    shdr = tryFindSectionHeader(".rela.IA_64.pltoff"); /* 64-bit Linux, IA-64 */
                ...
                if (shdr) {
                    ...
                    fprintf(stderr,
                            "warning: DT_MIPS_RLD_MAP_REL entry is present, but .rld_map section is not\n");
                    dyn->d_un.d_ptr = 0;
                }
            }
    }
        ...
        for (size_t entry = 0; (entry + 1) * sizeof(Elf_Sym) <= rdi(shdr.sh_size); entry++) {
            ...
            if (shndx != SHN_UNDEF && shndx < SHN_LORESERVE) {
                if (shndx >= sectionsByOldIndex.size()) {
                    fprintf(stderr,
                            "warning: entry %d in symbol table refers to a non-existent section, skipping\n",
                            shndx);
                    continue;
                }
...
void ElfFile<ElfFileParamNames>::modifySoname(sonameMode op, const std::string &newSoname,
            ...
            if (strlen(soname) == 0) {
                ...
                if (out)
                    *out = soname;
                    ...
                    debug("SonameMode is returnSoname, but out is nullptr\n");
            }
        } else {
            debug("no DT_SONAME found\n");
        }
    ...
    if (dynSoname) {
        ...
        unsigned int idx = 0;
        for (; rdi(reinterpret_cast<const Elf_Dyn *>(newDynamic.c_str())[idx].d_tag) !=
               DT_NULL; idx++);
...
template<ElfFileParams>
void ElfFile<ElfFileParamNames>::modifySoname(sonameMode op, const std::string &newSoname) {
    modifySoname(op, newSoname, nullptr);
}
...
template<ElfFileParams>
std::string
ElfFile<ElfFileParamNames>::shrinkRPath(char *rpath, std::vector<std::string> &neededLibs,
    ...
    for (auto &dirName: splitColonDelimitedString(rpath)) {
        ...
        for (unsigned int j = 0; j < neededLibs.size(); ++j)
            if (!neededLibFound.at(j)) {
                ...
                try {
                    Elf32_Half library_e_machine = getElfType(
                            readFile(libName, sizeof(Elf32_Ehdr))).machine;
...
void ElfFile<ElfFileParamNames>::modifyRPath(RPathOp op,
                                             const std::vector<std::string> &allowedRpathPrefixes,
                                             std::string newRPath, std::string *out) {
    auto shdrDynamic = findSectionHeader(".dynamic");
        ...
        case rpGet: {
            if (out)
                *out =  rpath ? rpath : "";
                ...
                debug("RPathOp is rpGet, but out is nullptr\n");
            return;
        }
    }
...
void ElfFile<ElfFileParamNames>::modifyRPath(RPathOp op,
                                             const std::vector<std::string> &allowedRpathPrefixes,
                                             std::string newRPath) {
    modifyRPath(op, allowedRpathPrefixes, newRPath, nullptr);
}
...
template<ElfFileParams>
void ElfFile<ElfFileParamNames>::printNeededLibs(std::set<std::string> *out) const {
...
void
ElfFile<ElfFileParamNames>::forAllStringReferences(const Elf_Shdr &strTabHdr, StrIdxCallback &&fn) {
...
template<ElfFileParams>
void
ElfFile<ElfFileParamNames>::setSoName(const std::string &newSoName) {
    modifySoname(replaceSoname, newSoName);
}

template<ElfFileParams>
std::string
ElfFile<ElfFileParamNames>::getSoName() {
    std::string ret;
    modifySoname(replaceSoname, "", &ret);
    return ret;
}

template<ElfFileParams>
void
ElfFile<ElfFileParamNames>::setRPath(const std::string &newRPath) {
    modifyRPath(rpSet, {}, newRPath);
}

template<ElfFileParams>
std::string
ElfFile<ElfFileParamNames>::getRPath() {
    std::string ret;
    modifyRPath(rpGet, {}, "", &ret);
    return ret;
}

template<ElfFileParams>
void
ElfFile<ElfFileParamNames>::addNeeded(const std::string &lib) {
...
template<ElfFileParams>
void
ElfFile<ElfFileParamNames>::removeNeeded(const std::string &lib) {
    std::set<std::string> libs;
    libs.insert(lib);
    removeNeeded(libs);
}
template<ElfFileParams>
std::set<std::string>
ElfFile<ElfFileParamNames>::getNeeded() const {
...
template<ElfFileParams>
int
ElfFile<ElfFileParamNames>::getEType() const {
    return rdi(hdr()->e_type);
}

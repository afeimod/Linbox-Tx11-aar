...
#define ElfFileParams class Elf_Ehdr, class Elf_Phdr, class Elf_Shdr, class Elf_Addr, class Elf_Off, \
                class Elf_Dyn, class Elf_Sym, class Elf_Versym, class Elf_Verdef, class Elf_Verdaux, \
                class Elf_Verneed, class Elf_Vernaux, class Elf_Rel, class Elf_Rela, unsigned ElfClass

#define ElfFileParamNames Elf_Ehdr, Elf_Phdr, Elf_Shdr, Elf_Addr, Elf_Off, Elf_Dyn, Elf_Sym, \
                Elf_Versym, Elf_Verdef, Elf_Verdaux, Elf_Verneed, Elf_Vernaux, Elf_Rel, Elf_Rela, ElfClass
...
public:
    ...
    virtual FileContents getFileContents() const = 0;

    [[nodiscard]] virtual bool isChanged() const noexcept = 0;

    virtual int getEType() const = 0;

    virtual void setInterpreter(const std::string &newInterpreter) = 0;
    virtual std::string getInterpreter() const = 0;

    virtual void setSoName(const std::string &newSoName) = 0;
    virtual std::string getSoName() = 0;

    virtual void setRPath(const std::string &newRPath) = 0;
    virtual std::string getRPath() = 0;

    virtual void addNeeded(const std::string &lib) = 0;
    virtual void removeNeeded(const std::string &lib) = 0;
    virtual std::set<std::string> getNeeded() const = 0;
};

template<ElfFileParams>
class ElfFile : public BaseElfFile {
...
private:
    ...
    [[nodiscard]] std::optional<std::reference_wrapper<const Elf_Shdr>>
    tryFindSectionHeader(const SectionName &sectionName) const;

    template<class T>
    span<T> getSectionSpan(const Elf_Shdr &shdr) const;

    template<class T>
    span<T> getSectionSpan(const SectionName &sectionName);

    template<class T>
    span<T> tryGetSectionSpan(const SectionName &sectionName);
...
public:
    ...
    typedef enum {
        printOsAbi, replaceOsAbi
    } osAbiMode;

    void modifyOsAbi(osAbiMode op, const std::string &newOsAbi);

    typedef enum {
        printSoname, replaceSoname, getSoname
    } sonameMode;

    void modifySoname(sonameMode op, const std::string &newSoname, std::string *out);
    ...
    typedef enum {
        rpPrint, rpShrink, rpSet, rpAdd, rpRemove, rpGet
    } RPathOp;

    void modifyRPath(RPathOp op, const std::vector<std::string> &allowedRpathPrefixes,
                     std::string newRPath, std::string *out);

    void modifyRPath(RPathOp op, const std::vector<std::string> &allowedRpathPrefixes,
                     std::string newRPath);

    std::string shrinkRPath(char *rpath, std::vector<std::string> &neededLibs,
    ...
    void printNeededLibs(std::set<std::string> *out) const;
...
public:
    FileContents getFileContents() const {
        return fileContents;
    };

    void setSoName(const std::string &newSoName);
    std::string getSoName();

    void setRPath(const std::string &newRPath);
    std::string getRPath();

    void addNeeded(const std::string &lib);
    void removeNeeded(const std::string &lib);
    std::set<std::string> getNeeded() const;

...
static void *
extract_group_icon_cursor_resource(WinLibrary *, WinResource *, char *, size_t *, bool);
...
extract_group_icon_cursor_resource(WinLibrary *fi, WinResource *wr, char *lang,
    ...
    for (c = 0; c < icondir->count; c++) {
        ...
        if (get_resource_entry(fi, fwr, &iconsize) != NULL) {
            ...
            if (iconsize != icondir->entries[c].bytes_in_res) {
                warn(_("%s: mismatch of size in icon resource `%s' and group (%d vs %d)"), fi->name,
                     name, iconsize, icondir->entries[c].bytes_in_res);
            }
            size += iconsize < icondir->entries[c].bytes_in_res ? icondir->entries[c].bytes_in_res
                                                                : iconsize;
    ...
    offset = sizeof(Win32CursorIconFileDir) +
             (icondir->count - skipped) * sizeof(Win32CursorIconFileDirEntry);
    size += offset;
    *ressize = size;
    ...
    for (c = 0; c < icondir->count; c++) {
        ...
        if (!is_icon) {
            ...
            fileicondir->entries[c - skipped].height =
                    icondir->entries[c].res_info.cursor.height / 2;

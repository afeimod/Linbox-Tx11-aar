...
strbuf_new_from_substring_n(size_t times, const char *substr, ssize_t subsp, ssize_t subep) {
    ...
    if (subsp > subep) SWAP_SSIZE_T(subsp, subep);
...
void
strbuf_replace_data_n(StrBuf *sb, ssize_t sp, ssize_t ep, size_t times, const void *mem,
                      size_t len) {
    size_t addlen;
    size_t dellen;
...
void
strbuf_replace_substring_n(StrBuf *sb, ssize_t sp, ssize_t ep, size_t times, const char *substr,
                           ssize_t subsp, ssize_t subep) {
    subsp = normalize_str_pos(substr, subsp);
    subep = normalize_str_pos(substr, subep);
    if (subsp > subep) SWAP_SSIZE_T(subsp, subep);

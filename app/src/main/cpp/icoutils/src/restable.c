...
static void
do_resources_recurs(WinLibrary *, WinResource *, WinResource *, WinResource *, WinResource *,
                    const char *, const char *, const char *, DoResourceCallback, const void *);
...
void
do_resources(WinLibrary *fi, const char *type, const char *name, const char *lang,
             DoResourceCallback cb, const void *data) {
    WinResource *type_wr;
    WinResource *name_wr;
    WinResource *lang_wr;
    ...
    do_resources_recurs(fi, NULL, type_wr, name_wr, lang_wr, type, name, lang, cb, data);
...
do_resources_recurs(WinLibrary *fi, WinResource *base, WinResource *type_wr,
    ...
    int c, rescnt;
    WinResource *wr;
    MethodParams *mp = data ? (MethodParams *) data : NULL;
    ...
    for (c = 0; c < rescnt; c++) {
        ...
        if (LEVEL_MATCHES(type) && LEVEL_MATCHES(name) && LEVEL_MATCHES(lang)) {
            if (wr->is_directory) {
                ...
                if (mp && mp->cbTimes != 0 && mp->logoOnly)
                    return;

                cb(fi, wr + c, type_wr, name_wr, lang_wr, data);
...
list_ne_type_resources(WinLibrary *fi, int *count) {
    ...
    for (rescnt = 0; typeinfo->type_id != 0; rescnt++) {
        if (((char *) NE_TYPEINFO_NEXT(typeinfo)) + sizeof(uint16_t) >
            fi->memory + fi->total_size) {
...
read_library(WinLibrary *fi) {
    ...
    if (PE_HEADER(fi->memory)->signature == IMAGE_NT_SIGNATURE) {
        ...
        for (d = pe_header->file_header.number_of_sections - 1; d >= 0; d--) {
            ...
            RETURN_IF_BAD_OFFSET(0, fi->memory + pe_sec->pointer_to_raw_data,
...
WinResource *
find_resource(WinLibrary *fi, const char *type, const char *name, const char *language,
              int *level) {
    WinResource *wr;

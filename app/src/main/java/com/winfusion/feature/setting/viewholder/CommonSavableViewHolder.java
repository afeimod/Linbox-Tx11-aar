...
public class CommonSavableViewHolder extends CommonViewHolder<SavableModel> {

    public CommonSavableViewHolder(@NonNull ListItemSettingsCommonBinding binding,
    ...
    @Override
    public void bind(@NonNull SavableModel model) {
        super.bind(model);
        if (model.hasFlag(FLAG_SHOW_DETAILS)) {
            binding.textSettingDetails.setVisibility(VISIBLE);
            binding.textSettingDetails.setText(
                    model.getDetailsFormatter().getFormattedValue(model.getValue()));
        } else {
            binding.textSettingDetails.setVisibility(GONE);
        }
        binding.buttonClear.setVisibility(model.canFollowed() ? VISIBLE : GONE);
